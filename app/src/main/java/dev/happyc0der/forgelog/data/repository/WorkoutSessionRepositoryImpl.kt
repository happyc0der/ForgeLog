...
class WorkoutSessionRepositoryImpl @Inject constructor(
     ...
     */
    override suspend fun appendSetLog(setLog: SetLog): Long = withContext(ioDispatcher) {
        database.withTransaction {
            val number = workoutSessionDao.nextSetNumber(setLog.sessionExerciseId)
            workoutSessionDao.upsertSetLog(setLog.copy(id = 0L, setNumber = number).toEntity())
        }
    }
